...
from utils.chart_analysis import fetch_closing_prices, get_support_resistance_zones
...
class GenerateCommand(app_commands.Command):
    ...
    async def callback(self, interaction: discord.Interaction, network: str, pair: str):
        try:
            ...
            zones = get_support_resistance_zones(symbol)

            if zones:
                support = zones["support"]
                resistance = zones["resistance"]
                range_span = resistance - support if resistance > support else 0.01
                passive_apr = round((info["volume_usd"] / info["liquidity_usd"]) * (range_span / info["price_usd"]) * 100, 2)
                price_range = {
                    "lower": support,
                    "upper": resistance,
                    "confidence": "Support/Resistance zones based on price density"
                }
